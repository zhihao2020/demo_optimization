...
def _cfg(**term_overrides):
    """构造带终端 SOC 配置的 RewardCalculator 配置字典。

    Args:
        **term_overrides: 覆盖 terminal_soc 子字段。
...
def test_reward_formula_uses_fmu_total_delta():
    """验证 reward 为 economic_cashflow_total 增量除以 C_ref，非 Python 侧重定价。"""
...
def test_terminal_bonus_only_on_complete_satisfied():
    """验证完整 episode 且 SOC 满足时终端 bonus 才计入总 reward。"""
    calc = RewardCalculator(_cfg())
    calc.reset(_out())
...
def test_quadratic_terminal_mode_is_exclusive():
    """验证 quadratic_penalty 模式使用二次惩罚而非 binary bonus 数值。"""
