...
Greedy 输出 `caes_mode_onehot=[1,0,0]`（放电）且 `mag=1`，动态区间上沿是 `-0.33`。float32 插值得到 `-0.32999998`，`project_u_caes` 把它当成空隙扫成 idle。当时冷罐温度刚好让 idle 被 mask，GiveSafe 一次拒绝 → `failed_no_safe_action`。这不是训练不够。

修复：`mag∈{0,1}` 时直接取区间端点；`project_u_caes(_torch)` 对端点 ULP 做 snap，不再把 `-0.32999998` 扫成 0。真空隙（如 0.2）仍投影到 idle。

C2 的 6 次 false-safe 是 Oracle 放行后 FMU 越界；其中一部分可能也是「本该放电却被扫成 idle」。修完后需新目录重跑 Stage C 才能再过门。
...
3. `FeasibilityOracle.check_action_executable`：同一 helper，去掉硬编码 `1e-9`。**IDLE 被 mask 禁止**时直接拒绝；即使传入过期 `idle=True` 的 feasible，也再用 `_caes_idle_step_ok` 拦一层，**不用** `predict_next_state(u_caes=0)` 放行 idle。snap 后的 `u_checked` 才进入后续预测。
4. `_caes_idle_step_ok`：不用 `predict_next_state(u_caes=0)`。idle 用 residual P99 + margin 的双边 envelope（气/热/冷 SOC、压力、有限温度侧）。`6.685 MPa` 由 `gas_pressure_min_Pa + residual_p99_pressure + margin_pressure_Pa` 算出，不硬编码。
5. idle 被禁时不回退成 idle。无合法 mode → `feasible_set_empty`。
6. 文档 critic 与代码对齐：3-D \(Q(s,u^{\mathrm{th}},u^{\mathrm{bat}},u_{\mathrm{caes}})\)，lr \(3\times10^{-4}\)。未把 critic 改回 6-D。

## 测试

本地 `pytest`：endpoint / idle / mode_mask / illegal_no_fmu / pc_hybrid / joint / givesafe_deterministic / battery / phase_d5 **56 passed**。新增：过期 mask 仍拒 idle；6.60 MPa 下 `env.step(idle)` 的 `FakeAdapter.step_calls` 不增加。

## 修复前后（代码层，非 Stage C 数字）

| 项 | 修前 | 修后 |
|----|------|------|
| `0.93000000715` vs `[0.86, 0.9299999999999999]` | Oracle `1e-9` 误拒 | endpoint snap，不当拒绝 |
