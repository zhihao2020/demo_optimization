...
| 字段 | 主值 | 等级 | 出处与边界 |
|------|------|------|------------|
| 分时时段与浮动比例 | 五档 S/P/F/V/D | M | 鲁发改价格〔2023〕914 号 + 国网山东 2026 工商业分时公告（机制） |
| 到户购电绝对值 | 1.136/0.986/0.635/0.284/0.183 | S+M | 由 `proxy+capacity_comp+line_loss+sys_op` 浮动基 + 110 kV 输配电度价 + 基金 **构造**；**不得**整体称作“官方 2026 实收电价” |
| `td_energy` 0.106 | CNY/kWh | O/M | NDRC 第四监管周期省级输配电价 · 山东 110 kV 档算例 |
| 售电 0.1875 | CNY/kWh | S | \(0.5\times0.225+0.5\times0.15\) 平坦出口价；illustrative |

论文口径：**山东分时机制约束下的构造性到户价（proxy-purchase path）**，非 ISO 出清、非当月代理购电真值。
