...
def main(summary_path: str = "runs/feasibility_probe/summary.json") -> None:
    ...
    if "battery_soc" in stats:
        ...
        bat["residual_p99_charge_high"] = max(
            float(bat.get("residual_p99_charge_high", 0)),
            float(stats["battery_soc"]["p99"]),
        )
    ...
    if "caes_gas_soc" in charge_stats:
        ...
        chg["residual_p99_gas_high"] = max(
            float(chg.get("residual_p99_gas_high", 0)),
            float(charge_stats["caes_gas_soc"]["p99"]),
        )
    ...
    for key, dest, field in (
        ...
        if key in charge_stats:
            ...
            dest[field] = max(
                float(dest.get(field, 0)), float(charge_stats[key]["p99"])
            )
    ...
    for key, dest, field in (
        ...
        if key in discharge_stats:
            ...
            dest[field] = max(
                float(dest.get(field, 0)), abs(float(discharge_stats[key]["min"]))
            )
